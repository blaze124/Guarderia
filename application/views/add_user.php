...
<!-- 
	...
	{
		if(document.alta_usuario.ROL[0].checked==true || document.alta_usuario.ROL[1].checked==true)
		{
			document.getElementById('no_alum').style.display='block';
			document.getElementById('alum').style.display='none';	
		}
		else if(document.alta_usuario.ROL[2].checked==true)
		{
...
<?php
	...
	$datos= array('name'=>'alta_usuario');
	
	echo form_open('index.php/main_controller/alta_usuario',$datos);
	...
	echo "<input type='date' name='fnac' placeholder=' Fecha de nacimiento: dd/mm/aaaa'/>";echo form_error('fnac');
	echo '<br>';
	...
	echo '<div id="alum" style="display:block;">
		...
		<br>
		<p>Aula Matinal</p>
		<input type="radio" name="matinal" value=1 > Sí <br>
		<input type="radio" name="matinal" value=0  checked > No <br>
		<br>
		<p>Forma de pago</p>
